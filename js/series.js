...
async function loadSeries() {
  try {
    const url = `${baseURL}/tv/popular?api_key=${apiKey}&language=ar&page=1`;
    const data = await fetch(url).then(res => res.json());
    seriesData = data.results.map(s => ({
      ...s,
      titleFinal: s.name || "-",
      posterFinal: s.poster_path || ""
    }));
    renderPage(currentPage);
  } catch(err) {
    console.error("خطأ في تحميل المسلسلات:", err);
    contentGrid.innerHTML = "<p>فشل تحميل المسلسلات</p>";
  }
}
...
function renderPage(page) {
  ...
  pageData.forEach(item => {
    ...
    card.addEventListener("click", () => {
      window.location.href = `details.html?type=series&id=${item.id}`;
    });
...
function applySearchAndFilter() {
  ...
  return seriesData.filter(s => {
    const matchesSearch = s.titleFinal.toLowerCase().includes(searchText);
    const matchesFilter = filterValue ? s.genre_ids?.includes(Number(filterValue)) : true;
